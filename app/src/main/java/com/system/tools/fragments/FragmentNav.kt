...
import com.system.kr.IMGKR
import com.system.kr.KrScriptConfig
import com.system.library.shell.BatteryUtils
import com.system.permissions.CheckRootStatus
import com.system.shell_utils.BackupRestoreUtils
import com.system.utils.AccessibleServiceHelper
import com.system.tools.R
import com.system.tools.activities.*
import com.system.tools.dialogs.DialogXposedGlobalConfig
import com.system.xposed.XposedCheck
import com.system.tools.OpenPageHelper
...
class FragmentNav : Fragment(), View.OnClickListener {
    ...
    private fun tryOpenApp(packageName: String) {
        ...
        if (packageName.equals("com.system.gesture")) {
            try {
                val intent = Intent(Intent.ACTION_VIEW)
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                intent.setComponent(ComponentName("com.system.gesture", "com.system.gesture.SettingsActivity"))
                startActivity(intent)
                return
            } catch (ex: java.lang.Exception) {
            }
        } else if (packageName.equals("com.system.filter")) {
            try {
                val intent = Intent(Intent.ACTION_VIEW)
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                intent.setComponent(ComponentName("com.system.filter", "com.system.filter.SettingsActivity"))
                startActivity(intent)
                return
            } catch (ex: java.lang.Exception) {
            }
    ...
    override fun onClick(v: View?) {
        v?.run {
            ...
            when (id) {
                R.id.nav_freeze -> {
                    val intent = Intent(Intent.ACTION_VIEW)
                    intent.setClassName("com.system.tools", "com.system.tools.activities.ActivityFreezeApps2")
                    startActivity(intent)
                    return
                }
                ...
                R.id.nav_charge -> {
                    ...
                    R.id.nav_img -> {
                    val IMGKR = IMGKR().init(context!!)
                    val activity = activity!!
                    IMGKR.pageListConfig?.run {
                ...
                R.id.nav_gesture -> {
                    tryOpenApp("com.system.gesture")
                    return
                }
                R.id.nav_filter -> {
                    tryOpenApp("com.system.filter")
                    return
                }
                R.id.nav_processes -> {
    ...
    private fun xposedCheck(onPass: Runnable) {
        ...
        try {
            vAddinsInstalled = context!!.packageManager.getPackageInfo("com.system.vaddin", 0) != null
        } catch (ex: Exception) {
